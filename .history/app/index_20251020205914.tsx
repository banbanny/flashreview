...
export default function IndexScreen() {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(collection(db, 'users', user.uid, 'reviewers'));
    const unsub = onSnapshot(q, (snapshot) => {
      const items: Reviewer[] = snapshot.docs.map(d => ({ id: d.id, ...(d.data() as any) }));
      setReviewers(items);
    }, (err) => {
          ...
          <View style={styles.card}>
            ...
            <View style={styles.btnRow}>
              <TouchableOpacity
                ...
                onPress={() => router.push({ pathname: '/reviewscreen', params: { questions: JSON.stringify(item.questions) } })}
              >
