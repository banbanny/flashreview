...
export default function IndexScreen() {
  ...
  const handleLogout = async () => {
    try { await signOut(auth); router.replace('/login'); Alert.alert('Logged Out'); }
    catch (error) { console.error(error); Alert.alert('Error', 'Failed to sign out'); }
  };
  ...
  const deleteReviewer = async (docId: string) => {
    ...
    Alert.alert('Confirm Delete', 'Are you sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await deleteDoc(doc(db, 'users', user.uid, 'reviewers', docId));
          setReviewers(prev => prev.filter(r => r.firestoreId !== docId));
        } catch (error) { console.error(error); Alert.alert('Error', 'Failed to delete reviewer'); }
      } }
    ]);
  };
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <View style={styles.btnRow}>
              <TouchableOpacity style={styles.reviewBtn} onPress={() => router.push({ pathname: '/reviewscreen', params: { questions: JSON.stringify(item.questions) } })}>
