...
import { Reviewer, Question } from '../lib/types';
...
export default function AddScreen() {
  ...
  const [title, setTitle] = useState(editReviewer?.title || '');
  const [question, setQuestion] = useState('');
  const [answer, setAnswer] = useState('');
  const [questions, setQuestions] = useState<Question[]>(editReviewer?.questions || []);
  ...
  const saveReviewer = async () => {
    ...
    try {
      ...
      if (editReviewer?.firestoreId) {
        await updateDoc(doc(reviewersRef, editReviewer.firestoreId), {
          title,
          questions,
          updatedAt: new Date(),
        });
      } else {
        await addDoc(reviewersRef, {
          title,
          questions,
          createdAt: new Date(),
        });
