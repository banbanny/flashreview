...
useFocusEffect(
 ...
 const handleReview = async () => {
  ...
  // Use editingId if editing, else generate new ID
  const id = editingId || (parsed.find(s => s.title === title)?.id) || Math.random().toString();
  ...
  let updatedSets;
  if (editingId) {
    updatedSets = parsed.map((set: any) => (set.id === editingId ? newSet : set));
  } else {
    updatedSets = [...parsed, newSet];
  }
