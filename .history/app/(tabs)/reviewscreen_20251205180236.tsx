...
export default function ReviewScreen() {
 ...
 const handleDelete = (item: Reviewer) => {
  console.log("handleDelete called for:", item.firestoreId);
  if (!item.firestoreId) {
    Alert.alert("Error", "Reviewer ID is missing.");
    return;
  }

  if (alertShownRef.current.has(item.firestoreId)) {
    return; // Alert already shown for this item
  }

  alertShownRef.current.add(item.firestoreId);
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      setDeletingId(null);
      alertShownRef.current.delete(item.firestoreId!);
    }
  };
  ...
  Alert.alert(
    ...
    [
      { text: "Cancel", style: "cancel", onPress: () => alertShownRef.current.delete(item.firestoreId) },
            ...
            <View style={styles.btnRow}>
              ...
              >
                <Text style={styles.btnText}>Edit</Text>
                        ...
                        <TouchableOpacity
              onPress={() => {
                console.log("Delete pressed:", item.firestoreId);
                handleDelete(item);
              }}
