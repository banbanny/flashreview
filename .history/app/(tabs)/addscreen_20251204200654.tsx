...
import { saveReviewer, updateReviewer } from "../../lib/reviewers"; // <-- adjust path if needed
...
export default function AddScreen() {
      ...
      const loadData = async () => {
        try {
          ...
          if (editing) {
            ...
            // the file you gave earlier used parsed.firestoreId
            setEditingId(parsed.firestoreId || null);
          } else {
            setTitle("");
            setQuestions([]);
            setEditingId(null);
          ...
          console.warn("Error loading editingSet:", err);
        }
      };
      loadData();
    }, [])
  ...
  const addQuestion = () => {
    ...
    setQuestions((prev) => [
      ...prev,
      { id: Math.random().toString(36).slice(2), question: currentQuestion.trim(), answer: currentAnswer.trim() },
    ]);
    setCurrentQuestion("");
    setCurrentAnswer("");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        } as Omit<Reviewer, "firestoreId">);
        console.log("Saved new reviewer id:", newId);
          ...
          <View style={styles.questionsList}>
            ...
            {questions.map((q) => (
              <View key={q.id} style={styles.questionItem}>
                ...
                <Text style={[styles.qText, { fontSize: 13, color: "#5a3f00" }]}>→ {q.answer}</Text>
              </View>
            ))}
          </View>
        )}
