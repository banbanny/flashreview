...
export default function ReviewScreen() {
 ...
 const loadSets = async (uid: string) => {
  try {
    ...
    console.log("Loaded reviewers:", mine);
    setSets(mine);
  } catch (error) {
 ...
 const handleDelete = async (firestoreId?: string) => {
  ...
  Alert.alert("Confirm delete", "Are you sure you want to delete this reviewer?", [
    { text: "Cancel", style: "cancel" },
    {
      ...
      onPress: async () => {
        try {
          await deleteReviewer(firestoreId, user.uid);
...
const styles = StyleSheet.create({
  ...
  emptyText: { color: "#7A4D00", fontSize: 18, fontWeight: "bold", textAlign: "center" },
  subText: { color: "#A36A00", marginTop: 5, textAlign: "center" },
  card: {
