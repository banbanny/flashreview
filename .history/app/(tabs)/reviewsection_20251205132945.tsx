...
export default function ReviewSection() {
  ...
  const [feedback, setFeedback] = useState(""); // shows "Correct!" or "Wrong!"
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const all = await loadReviewers(user.uid); // ✅ pass uid
  ...
  const handleMark = (userAnswer: string) => {
    if (!current) return;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {feedback ? <Text style={{ marginTop: 12, fontSize: 16, fontWeight: "bold", color: feedback === "Correct!" ? "green" : "red" }}>{feedback}</Text> : null}

        <View style={styles.row}>
          <TouchableOpacity style={[styles.smallBtn, { backgroundColor: "#219EBC" }]} onPress={() => setShowAnswer((s) => !s)}>
            <Text style={styles.smallBtnText}>{showAnswer ? "Hide" : "Show Answer"}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.smallBtn, { backgroundColor: "#2E8B57" }]} onPress={() => handleMark(current.answer)}>
            <Text style={styles.smallBtnText}>I was correct</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.smallBtn, { backgroundColor: "#FF6347" }]} onPress={() => handleMark("wrong")}>
