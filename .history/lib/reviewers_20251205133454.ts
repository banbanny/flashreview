...
// ✅ Load all reviewers from Firestore
...
export const deleteReviewer = async (firestoreId: string, uid: string) => {
  if (!firestoreId || !uid) throw new Error("Missing reviewer ID or user UID");

  try {
    // First, delete all questionnaires inside the reviewer
    const questionnairesCol = collection(db, "reviewers", uid, "questionnaires");
    const qSnap = await getDocs(questionnairesCol);

    const deletePromises = qSnap.docs.map((d) => deleteDoc(doc(db, "reviewers", uid, "questionnaires", d.id)));
    await Promise.all(deletePromises);

    // Then delete the reviewer document itself
    await deleteDoc(doc(db, "reviewers", uid));
  } catch (err) {
